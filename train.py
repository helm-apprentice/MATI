...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='procontext', help='training script name')
...
def main():
    ...
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --distill %d --script_teacher %s --config_teacher %s --use_wandb %d > warnings.log 2>&1"\
