...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if 'fps' in output:
        fps_data = np.array(output['fps']).astype(float)
        fps_file = '{}_fps.txt'.format(base_results_path)
        np.savetxt(fps_file, fps_data, delimiter='\t', fmt='%f')
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    # if _results_exist() and not debug: # 如果结果文件已经存在并且不是调试模式
    #     print('FPS: {}'.format(-1)) # 打印FPS为-1的信息（表示不需要计算FPS）
    #     return
    ...
    # 将FPS信息加入到output中，用于保存
    output['fps'] = [fps] * num_frames  # 创建一个每帧都是平均FPS值的列表
    if not debug:
