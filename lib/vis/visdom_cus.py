...
class Visdom:
    ...
    # def attn_to_image(self, attn_weights, img_size=(192, 192)):
    #     """
    #     将注意力权重转换为PIL图像。

    #     参数:
    #     attn_weights -- 注意力权重张量，形状为 [heads, H, W]
    #     img_size -- 目标图像大小
    ...
    #     """
    #     # 计算所有头的平均注意力
    #     attn = attn_weights.detach().mean(dim=0).cpu()  # 现在 attn 的形状为 [H, W]

    #     # 将注意力映射调整大小到模板图像的大小
    #     attn = torch.nn.functional.interpolate(attn.unsqueeze(0).unsqueeze(0), size=img_size, mode='bilinear', align_corners=False)
    #     attn = attn.squeeze(0).squeeze(0)

    #     # 转换为PIL图像
    #     attn_image = to_pil_image(attn)

    #     return np.array(attn_image)
    #     # return attn_image
    def attn_to_image(self, attn_weights, img_size=(192, 192)):
        # Calculate the average attention across all heads
        attn = attn_weights.mean(dim=0).cpu()

        # Resize the attention map to the desired image size
        attn_image = torch.nn.functional.interpolate(attn.unsqueeze(0).unsqueeze(0),
                                                    size=img_size, mode='bilinear', align_corners=False).squeeze()

        # Normalize the attention image to [0, 1] and convert to a 3-channel image for visualization
        attn_image = attn_image - attn_image.min()
        attn_image = attn_image / attn_image.max()
        attn_image = torch.stack([attn_image, attn_image, attn_image], dim=0)  # Convert to 3-channel image

        return attn_image

    def plot_attention_heatmap(self, attn_weights, img_size=(192, 192)):
        """
        使用matplotlib创建一个注意力热图并通过Visdom显示。
        ...
        """
        # 将注意力权重转换为图像
        attn_image = self.attn_to_image(attn_weights, img_size)

        # 将注意力图像转换为热图
        fig, ax = plt.subplots()
        cax = ax.matshow(attn_image[0].numpy(), cmap='viridis')
        fig.colorbar(cax)
        ...
        buf.seek(0)
        attn_heatmap = Image.open(buf)

        # 显示图像和热图
        # plt.close(fig)  # 关闭图表以防止它显示在Jupyter笔记本或Python脚本中
        return np.array(attn_heatmap)
    # return attn_heatmap
    # vis.image(np.array(attn_heatmap), opts=dict(title='Attention Heatmap'))
    

# 示例使用函数
# 假设你有一个形状为 [144, 32, 32] 的PyTorch张量 attn_weights
# attn_weights = ...

# 转换为图像
# attn_img = attn_to_image(attn_weights)
# vis.image(np.array(attn_img), opts=dict(title='Attention Map'))
