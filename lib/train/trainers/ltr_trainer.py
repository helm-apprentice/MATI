...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        # assert isinstance(optimizer, torch.optim.Optimizer), "self.optimizer must be an instance of torch.optim.Optimizer"
        # # assert isinstance(self.optimizer, torch.optim.Optimizer), "self.optimizer must be an instance of torch.optim.Optimizer"
        # print(f"optimizer.ltr_init: {optimizer}")
        # print(f"optimizer.ltr_init_self: {self.optimizer}")
    ...
    def cycle_dataset(self, loader):
        ...
        self._init_timing()
        # print('trainer.py  line73')
        #print(loader)
        # 临时测试数据加载器
        # try:
        #     # 获取数据加载器的第一个批次数据
        #     first_batch_data = next(iter(loader))
        ...
        for i, data in enumerate(loader, 1):
            # print('i: ', i)
            # print('trainer_data: ', data)
            self.data_read_done_time = time.time()
            # get inputs
            if self.move_data_to_gpu:
                # ===========================================================
                # print(f'trainer_line92_self.device: {self.device}')
                ...
                #                 print(f"{key}[{subkey}]: {subvalue.device}")
                # ===========================================================
            ...
            # forward pass
            if not self.use_amp:
                loss, stats, out_dict = self.actor(data)
                # print(f"out_dict: {out_dict.keys()}")
                # print(f"out_dict.attn: {out_dict['attn'][0].shape}")
                # print(f"out_dict.attn_cmc: {out_dict['attn_cmc'].shape}")
                """ out_dict.attn[0]: torch.Size([32, 12, 400, 400])
                    out_dict.attn_cmc: torch.Size([144, 32, 32]) """
            else:
                with autocast():
                    loss, stats, out_dict = self.actor(data)
            ...
            if loader.training:
                # print(i)
                # print(f"optimizer.keys: {self.optimizer.keys()}")
                # assert isinstance(self.optimizer, torch.optim.Optimizer), "self.optimizer must be an instance of torch.optim.Optimizer"
            ...
            # update statistics
            batch_size = data['template_images']['visible'].shape[loader.stack_dim]
            ...
            if self.wandb_writer is not None and i % self.settings.print_interval == 0:
                if self.settings.local_rank in [-1, 0]:
                    ...
                    if 'attn_cmc' in out_dict:
                        cmc_label = f"CMC Attention Epoch {self.epoch}"
                        log_data[cmc_label] = self.wandb_writer.attn_to_image(out_dict['attn_cmc'])
                    if 'attn' in out_dict:
                        for j, attn in enumerate(out_dict['attn']):
                            ce_label = f"CE{j} Attention Epoch {self.epoch}"
                            log_data[ce_label] = self.wandb_writer.attn_to_image(attn)
                    # =========================================================================================
                    self.wandb_writer.write_log(self.stats, self.epoch)
                    self.wandb_writer.write_image_log(log_data, self.epoch)
    ...
    def train_epoch(self):
        ...
        for loader in self.loaders:
            ...
            if self.epoch % loader.epoch_interval == 0:
                """
                val的loader有另外设置epoch_interval,所以val会以这个间隔出现,而train没有设置间隔,会每轮都出现
                train在创建LTRloader时有个默认属性epoch_interval为1
                """
                # 2021.1.10 Set epoch
    ...
    def move_to_device(self, data, device):
        """
        move data to device aimed
        """
        for key, value in data.items():
            if isinstance(value, torch.Tensor):
                data[key] = value.to(device)
            elif isinstance(value, dict):
                data[key] = self.move_to_device(value, device)
        return data