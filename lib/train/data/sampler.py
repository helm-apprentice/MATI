...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        self.processing = processing  # processing.STARKProcessing()
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':  # 用的就是这个
            ...
            try:
                ...
                # print('point')
                # print(f'template_frames: {template_frames}')
                img_types = ['visible', 'infrared'] if template_frames['infrared'] else ['visible']
                #self.use_infrared = True if data['use_infrared'] else False
                H = {'visible': [], 'infrared': []}
                W = {'visible': [], 'infrared': []}
                template_masks = {'visible': [], 'infrared': []}

                for img_type in img_types:
                    #print(f'template_frames[{img_type}][0].shape: {template_frames[img_type][0].shape}')
                    H[img_type], W[img_type], _ = template_frames[img_type][0].shape
                    # print('point2')
                    template_masks[img_type] = template_anno[img_type]['mask'] if 'mask' in template_anno \
                                                else [torch.zeros((H[img_type], W[img_type]))] * self.num_template_frames

                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H['visible'], W['visible']))] * self.num_search_frames
                # print('point3')
                data = TensorDict({'template_images': {'visible': template_frames['visible'],
                                                       'infrared': template_frames['infrared']},  # list
                                   'template_anno': {'visible': template_anno['visible_bbox'],
                                                    'infrared': template_anno['infrared_bbox']},
                                   'template_masks': {'visible': template_masks['visible'],
                                                      'infrared': template_masks['infrared']},
                                   'search_images': search_frames['visible'],
                                   'search_anno': search_anno['visible_bbox'],
                                   'search_masks': search_masks,
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name')})
                # print(f'pre_process_data: {data}')
                # make data augmentation
                data = self.processing(data)
                # print(f'after_process_data: {data}')
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                # In case of image dataset, just repeat the image to generate synthetic video
                template_frame_ids = [1] * self.num_template_frames  # 1
                search_frame_ids = [1] * self.num_search_frames
            try:
